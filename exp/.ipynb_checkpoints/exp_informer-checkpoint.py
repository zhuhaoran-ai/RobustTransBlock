...
class Exp_Informer(Exp_Basic):
    ...
    def predict(self, args, setting, load=False):
        pre_data = pd.read_csv(args.root_path + args.data_path)[-1500:]
        ...
        for i in range(int(len(pre_data)/pre_length)):  # 一次预测一周的数据
            if i == 0:
                pred_data, pred_loader = self._get_data(flag='pred')
                for j in range(pre_length):
                    slreals.append(pre_data['sl'][i*pre_length + j])
                    # offeredreals.append(pre_data['offered'][i*pre_length + j])
                    # Fofferedreals.append(pre_data['Foffered'][i*pre_length + j])
                    # ASAreals.append(pre_data['ASA'][i*pre_length + j])
            else:
                pred_data, pred_loader = self._get_data(flag='pred', pre_data=pre_data[['date','sl']].iloc[: i*pre_length])
                for j in range(pre_length):
                    slreals.append(pre_data['sl'][i*pre_length + j])
                    # offeredreals.append(pre_data['offered'][i*pre_length + j])
                    # Fofferedreals.append(pre_data['Foffered'][i*pre_length + j])
            ...
            for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(pred_loader):
                ...
                for i in range(pre_length):
                    slresults.append(pred[0][i][0].detach().cpu().numpy())
                    # ASAresults.append(pred[0][i][1].detach().cpu().numpy())
                    # offeredresults.append(pred[0][i][0].detach().cpu().numpy())
                    # Abandresults.append(pred[0][i][2].detach().cpu().numpy())
                    # staffresults.append(pred[0][i][3].detach().cpu().numpy())
                    # Handledresults.append(pred[0][i][4].detach().cpu().numpy())
                print(pred)  
...
#                            'offeredreal':offeredreals,'offeredresults':offeredresults,'Abandresults':Abandresults,'staffresults':staffresults,'Handledresults':Handledresults},index=range(len(slresults)))
#         df[['offeredforecast','ASAresults','slforecast']] = df[['offeredresults','ASAresults','slforecast']].astype('float')
...
#         test_features = df[['shift', 'slforecast','ASAresults',
#                  'offeredforecast','Abandresults','staffresults','Handledresults' ]]
...
#         df.loc[test_feature.index, 'slforecast'] = new_series
        shift = pre_data['date'][-1500:].tolist()
        
        df = pd.DataFrame({'date':shift, 'slreal': slreals, 'slforecast': slresults, 'slMAE': sllosss, 'slme': slMe})
