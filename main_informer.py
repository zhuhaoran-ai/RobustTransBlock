...
parser = argparse.ArgumentParser(description='Informer9 Long Sequences Forecasting')

parser.add_argument('--model', type=str, default='TimesNet',
                    help='model of experiment, options: [Informer]')
parser.add_argument('--data', type=str, default='custom', help='data')
parser.add_argument('--root_path', type=str, default='/home/zhuhaoran/', help='root path of the data file')
parser.add_argument('--data_path', type=str, default='88ce.csv', help='data file')
parser.add_argument('--is_rolling_predict', type=bool, default=True, help='rolling predict')
parser.add_argument('--rolling_data_path', type=str, default='88ce.csv', help='data file')
parser.add_argument('--features', type=str, default='M',
                    help='forecasting task, options:[M, S, MS]; M:multivariate predict multivariate, S:univariate predict univariate, MS:multivariate predict univariate')
parser.add_argument('--target', type=str, default='energy_heat_kwh', help='target feature in S or MS task  energy_heat_kwh')
parser.add_argument('--freq', type=str, default='h',
                    help='freq for time features encoding, options:[s:secondly, t:minutely, h:hourly, d:daily, b:business days, w:weekly, m:monthly], you can also use more detailed freq like 15min or 3h')
parser.add_argument('--checkpoints', type=str, default='/home/zhuhaoran/model/checkpoints/', help='location of model checkpoints')
...
parser.add_argument('--attn', type=str, default='full', help='attention used in encoder, optio---ns:[prob, full]')
...
for ii in range(args.itr):
    ...
    exp.train2(setting)#train2 is no,train is yes
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test(setting)#test2 is have,test is no have
    if args.do_predict:
